...
void MU_LogScopeValues(pDataSampleIQ Instant, pDataSampleIQ RMS, _iq CosPhi, Int16S PWM, Boolean SRAMDebug)
{
	...
	if(ScopeDivCounter++ >= ScopeDivCounterMax)
	{
		...
		else
		{
			...
			SampleToSave.U.Data.Current = _IQmpyI32int(Instant->Current, 1000);
			SampleToSave.U.Data.CurrentRMS = _IQmpyI32int(RMS->Current, 1000);
...
void MU_SaveSampleToEP(pDataSample Sample, Int16U Index)
{
	...
	MEMBUF_Values_ImA[Index] = Sample->U.Data.Current / 1000;
	MEMBUF_Values_IuA[Index] = Sample->U.Data.Current % 1000;

	MEMBUF_Values_Irms_mA[Index] = Sample->U.Data.CurrentRMS / 1000;
	MEMBUF_Values_Irms_uA[Index] = Sample->U.Data.CurrentRMS % 1000;

	MEMBUF_Values_CosPhi[Index] = Sample->U.Data.CosPhi;
...
void MU_InitCoeffX(pMeasureCoeff Coeff, Int16U StartRegister)
{
	...
	{
		case REG_COEFF_CURRENT2_K:
			P0div = 100;
			break;

		case REG_COEFF_CURRENT3_K:
			P0div = 1000;
			break;
	...
	Coeff->P0 = _FPtoIQ2((Int16S)DataTable[StartRegister + 4], P0div);
}
// ----------------------------------------
