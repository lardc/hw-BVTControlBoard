...
typedef enum __BatteryVoltageState {
	BVS_None = 0, BVS_WaitRise = 1, BVS_WaitFall = 2, BVS_Ready = 3
} BatteryVoltageState;
...
void CONTROL_Init() {
	...
	pInt16U EPCounters[EP_COUNT] = { (pInt16U) &MEMBUF_ValuesIV_Counter,
			(pInt16U) &MEMBUF_ValuesIV_Counter,
			(pInt16U) &MEMBUF_ValuesDIAG_Counter,
			(pInt16U) &MEMBUF_ValuesErr_Counter,
			(pInt16U) &MEMBUF_ValuesIVpeak_Counter,
			(pInt16U) &MEMBUF_ValuesIVpeak_Counter };
	pInt16U EPDatas[EP_COUNT] = { MEMBUF_Values_I, MEMBUF_Values_V,
			MEMBUF_Values_DIAG, MEMBUF_Values_Err, MEMBUF_Values_Ipeak,
			MEMBUF_Values_Vpeak };
	// Data-table EPROM service configuration
	EPROMServiceConfig EPROMService = { &ZbMemory_WriteValuesEPROM,
			&ZbMemory_ReadValuesEPROM };
...
void CONTROL_NotifyEndTest(_iq BVTResultV, _iq BVTResultI, Int16U DFReason,
		Int16U Problem, Int16U Warning) {
	...
	if (CurrentMeasurementType == MEASUREMENT_TYPE_DC
		...
		DataTable[REG_RESULT_I_UA_R] =
				(BVTResultI > 0) ? _IQmpyI32int(_IQfrac(BVTResultI), 1000) : 0;
	}
...
static void CONTROL_EndTestDPC() {
	...
	if (EndXDPCArgument.SavedDFReason != DF_NONE) {
		...
		DataTable[REG_FINISHED] =
				(EndXDPCArgument.SavedProblem == PROBLEM_NONE) ?
						OPRESULT_OK : OPRESULT_FAIL;
		DataTable[REG_WARNING] = EndXDPCArgument.SavedWarning;
		DataTable[REG_PROBLEM] =
				(EndXDPCArgument.SavedProblem == PROBLEM_OUTPUT_SHORT) ?
						PROBLEM_NONE : EndXDPCArgument.SavedProblem;
		...
		if (CONTROL_State == DS_InProcess) {
			*UserError = ERR_OPERATION_BLOCKED;
			...
			DEVPROFILE_ResetScopes(0,
					IND_EP_I | IND_EP_V | IND_EP_DBG | IND_EP_ERR
							| IND_EP_PEAK_I | IND_EP_PEAK_V);
...
static void CONTROL_TriggerMeasurementDPC() {
	...
	switch (CurrentMeasurementType) {
	case MEASUREMENT_TYPE_TEST:
		success = MEASURE_TEST_StartProcess(CurrentMeasurementType, &DFReason,
				&problem);
		break;
	case MEASUREMENT_TYPE_AC:
	case MEASUREMENT_TYPE_AC_D:
	case MEASUREMENT_TYPE_AC_R:
		success = MEASURE_AC_StartProcess(CurrentMeasurementType, &DFReason,
				&problem);
		break;
	case MEASUREMENT_TYPE_DC:
	case MEASUREMENT_TYPE_DC_STEP:
	case MEASUREMENT_TYPE_DC_RES:
		success = MEASURE_DC_StartProcess(CurrentMeasurementType, &DFReason,
				&problem);
		break;
	}
...
static void CONTROL_BatteryVoltagePrepare() {
	Int16U OutputPower = ((Int32U) DataTable[REG_LIMIT_VOLTAGE]
			* DataTable[REG_LIMIT_CURRENT]) / 10000;
	...
	if (DataTable[REG_USE_CUSTOM_PRIM_V]) {
		...
		if ((OutputPower > CAP_SW_POWER)
				|| (DataTable[REG_LIMIT_VOLTAGE] > CAP_SW_VOLTAGE)
			...
			if (DataTable[REG_ACTUAL_PRIM_VOLTAGE]
					> (DataTable[REG_PRIM_V_LOW_RANGE] + CAP_DELTA)) {
				DRIVER_SwitchPowerOff();
...
static void CONTROL_BatteryVoltageCheck() {
	Int16U FullRangeVoltage =
			(DataTable[REG_USE_CUSTOM_PRIM_V]) ?
					DataTable[REG_PRIM_V_CUSTOM] :
					DataTable[REG_PRIM_V_FULL_RANGE];

	switch (CONTROL_Battery) {
	case BVS_WaitRise: {
		if (DataTable[REG_ACTUAL_PRIM_VOLTAGE]
				>= (FullRangeVoltage - CAP_DELTA))
			CONTROL_BatteryVoltageReady(DataTable[REG_ACTUAL_PRIM_VOLTAGE]);
	}
		break;
