...
static void MEASURE_AC_HandleVI()
{
	...
	if(ActualSecondarySample.IQFields.Voltage > MaxPosVoltage)
	{
...
static void MEASURE_AC_CacheVariables()
{
	if (USE_DIRTY_PATCH && (DataTable[REG_LIMIT_CURRENT] == 3000))
		LimitCurrent = HVD_IH_TH;
	else
	...
	else if (LimitCurrent <= HVD_IH_TH)
	{
	...
	{
		LimitCurrentHaltLevel = HVD_IH2_TH;
