...
static Boolean DEVPROFILE_Validate16(Int16U Address, Int16U Data)
{
	...
	if(Address < DATA_TABLE_WR_START)
	{
		...
		{
			if(((Int16S)Data) < ((Int16S)(NVConstraint[Address - DATA_TABLE_NV_START].Min))
					|| ((Int16S)Data) > ((Int16S)(NVConstraint[Address - DATA_TABLE_NV_START].Max)))
				return FALSE;
		}
		else if(Data < NVConstraint[Address - DATA_TABLE_NV_START].Min
	...
	else if(Address < DATA_TABLE_WP_START)
	{
		...
		{
			if(((Int16S)Data) < ((Int16S)(VConstraint[Address - DATA_TABLE_WR_START].Min))
					|| ((Int16S)Data) > ((Int16S)(VConstraint[Address - DATA_TABLE_WR_START].Max)))
				return FALSE;
		}
		else if(Data < VConstraint[Address - DATA_TABLE_WR_START].Min
